...
- `session` - Active connections (Terminal/Signal/SMS) linked to players
...
├── session.rs          # Client session handling (Terminal, Signal, SMS)
...
✅ **SMS-friendly:** Accounts for high latency  
...
- **SMS Compatibility:** Works despite message delays
...
async fn main() {
    ...
    tokio::spawn(sms_listener(state.clone()));
...
- Add async network listeners (Terminal, Signal, SMS)
...
2. **Multi-Interface Support:** SMS has inherent latency, ticks normalize this
...
- Matches typical SMS round-trip time
